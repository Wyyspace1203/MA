...
class feat_bootleneck(nn.Module):
    ...
    def forward(self, x, pooling=False):  #view pooling for true -----------------------------------------------------
        x = self.bottleneck(x)
        if self.type == "bn":
            x = self.bn(x)
        ##------------view pooling -----------------------------------
...
class ActDomainNavigator(nn.Module):
    r"""Domain navigator model from
    Parameters:
    ...
    def __init__(self, in_feature: int, hidden_size: int, out_size: int):
        super(ActDomainNavigator, self).__init__()
